  ...
  describe "setting submit_host" do 
    ...
    context "when strict_host_checking = 'no' && submit_host specified" do
      it "defaults host checking to no" do
        ...
        batch.submit script.content
        expect(Open3).to have_received(:capture3).with(anything,'ssh', '-o', 'BatchMode=yes', '-o', 'UserKnownHostsFile=/dev/null', '-o', 'StrictHostKeyChecking=no', 'owens.osc.edu', 'qsub', any_args)
      end
    end
  end
end
